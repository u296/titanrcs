...
bool make_renderresources(RenderContext* ctx, CleanupStack* cs) {
    ...
    f = make_uniform_buffers(n_max_inflight, ctx->backend.physdev, ctx->backend.dev, &ctx->resources.ubufs, &ctx->resources.ubuf_mappings, &e, cs);
	CHECK

    f = make_commandbuffers(ctx->backend.dev,ctx->resources.cmd_pool,n_max_inflight,&ctx->resources.cmd_bufs, &e, cs);
	CHECK

	f = make_sync_objects(ctx->backend.dev, n_max_inflight,  &ctx->resources.img_ready_sems, &ctx->resources.render_finished_sems, &ctx->resources.inflight_fncs, &e,cs);
	CHECK
}