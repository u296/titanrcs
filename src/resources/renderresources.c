...
bool make_uniform_buffers(const u32 n_max_inflight, VkPhysicalDevice physdev, VkDevice dev,
                          Buffer** ubufs, void*** ubuf_mappings, Error* e_out, CleanupStack* cs) {

    *ubufs = malloc(sizeof(Buffer) * n_max_inflight);
    *ubuf_mappings = malloc(sizeof(void*) * n_max_inflight);

    for (u32 i = 0; i < n_max_inflight; i++) {
        make_buffer(physdev, dev, sizeof(UniformBufferObject), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
...
bool make_renderresources(RenderContext* ctx, CleanupStack* cs) {
    ...
    f = make_uniform_buffers(n_max_inflight, ctx->backend.physdev, ctx->backend.dev,
                             &ctx->resources.ubufs, &ctx->resources.ubuf_mappings, &e, cs);
    CHECK
