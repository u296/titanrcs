...
bool make_rcs_setup(RenderBackend* rb, VkCommandPool cpool,
...
#endif
#ifdef TR_CALCMODE_SUM
    make_rcs_downscale_descset_layout(rb->dev, &rcs_downscale_descset_layout,
                                      cs);
#endif
    ...
    for (u32 i = 0; i < N_MAX_INFLIGHT; i++) {
...
#ifdef TR_CALCMODE_FFT
        make_rcs_finalimg_fft(rb, ext, &rcs_inflights[i].fft_img, cs);
        ...
        make_rcs_intermediate_sum_img(
            rb, ext, &rcs_inflights[i].intermediate_downscale_img, cs);

        make_rcs_finalimg_sum(rb, ext, &rcs_inflights[i].fft_img, cs);
        ...
        make_rcs_reduction_descset(rb, rcs_dpool, rcs_inflights[i].fft_img,
    ...
    if (make_rcs_mesh(sharpfilename, rb, cpool, &rcs_sharp_mesh.vertexbuf,
                      ...
                      &rcs_sharp_mesh.sharpindexbuf,
                      &rcs_sharp_mesh.n_sharp_indices, cs)) {
