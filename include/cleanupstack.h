...
CLEANUP_START(PipelineCleanup) <---- PipelineCleanup is the type of the context given to the
destructor {dev, *pipeline}				<------ this is the value the context is
gonna be set to CLEANUP_END(pipeline)         <--- pipeline means that the destruction function is
called destroy_pipeline

*/
#define CLEANUP_START(cleanuptype)                                                                 \
    if (cs != NULL && r == VK_SUCCESS) {                                                           \
    cleanuptype cleanup_context =

#define CLEANUP_END(varname)                                                                       \
    ;                                                                                              \
    cs_push(cs, &cleanup_context, sizeof(cleanup_context), destroy_##varname);                     \
    }

// doesn't require result
#define CLEANUP_START_NORES(cleanuptype)                                                           \
    if (cs != NULL) {                                                                              \
    cleanuptype cleanup_context =

#define CLEANUP_START_ONORES(cleanuptype)                                                          \
    {                                                                                              \
        cleanuptype cleanup_context =

#define CLEANUP_END_O(varname)                                                                     \
    ;                                                                                              \
    cs_push(&cs, &cleanup_context, sizeof(cleanup_context), destroy_##varname);                    \
    }
